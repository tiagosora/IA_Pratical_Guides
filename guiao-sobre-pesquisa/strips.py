...
class STRIPS(SearchDomain):
    ...
    def result(self, state, action):
        for pred in action.pc:
            if pred in state and pred == False:
                return None

        newstate = [pred for pred in state if pred not in action.neg]
    ...
    def satisfies(self, state, goal):
        return all(pred in state for pred in goal)
