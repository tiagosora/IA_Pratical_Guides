# CP = Cara Preocupada
# PA = Precisa de Ajuda
# FR = Frequência do Rato
# SC = Sobrecarga de Trabalhos
...
class BayesNet:
    ...
    def individualProb(self, var, val):
        ...
        return sum([ self.jointProb([(var, val)] + conj) for conj in self.list_all_conjunctions(variaveis) ])

    def list_all_conjunctions(self, variaveis):
        if len(variaveis) == 1:
            return [ 
                [(variaveis[0], True)],
                [(variaveis[0], False)] 
        ...
        temp = []
        for c in self.list_all_conjunctions(variaveis[1:]):
            temp.append([(variaveis[0], True)] + c)
            temp.append([(variaveis[0], False)] + c)
